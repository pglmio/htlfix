  ...
  <div class="min-h-screen bg-orange-50 pb-20">
    ...
    <div class="p-4">
      <h2 class="font-bold text-gray-500 mb-4 uppercase text-sm flex justify-between items-center">
        Ticket Aperti
        <span class="badge badge-neutral text-xs">Live</span>
      </h2>
            ...
            <div class="flex justify-between items-start">
              <div>
                <div class="flex items-center gap-2 mb-1">
                  ...
                  <span v-if="isAssignedToMe(issue.assigned_to)" class="badge badge-primary text-xs font-bold animate-pulse">
                    PER TE 👤
                  </span>
                  <span v-else-if="issue.assigned_to" class="badge badge-ghost text-xs font-bold text-gray-400">
                    Per {{ issue.assigned_to }}
                  </span>
...
const fetchIssues = async () => {
  ...
  if (data && JSON.stringify(data) !== JSON.stringify(issues.value)) issues.value = data
}

const isAssignedToMe = (assignedName) => {
  return assignedName === myName
}

const fixIssue = async (id) => {
  if(!confirm('Confermi riparazione?')) return
  await supabase.from('issues').update({ status: 'fixed' }).eq('id', id)
  fetchIssues()
}

const formatDate = (dateString) => { return new Date(dateString).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }
const logout = () => { localStorage.clear(); router.push('/') }

onMounted(() => { fetchIssues(); pollingInterval = setInterval(fetchIssues, 1000) })
